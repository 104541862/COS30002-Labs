...
class World(object):
    ...
    def update(self, delta):
        ...
        if not self.paused:
            ...
            for projectile in self.projectiles:
                ...
                # collision check
                if self.target_agent and self.target_agent is not None and self.target_agent.alive:
                    if self.check_collision(projectile, self.target_agent):
                        self.target_agent.take_damage(projectile.damage)
            ...
            if self.target_agent and not self.target_agent.alive:
    ...
    def spawn_target(self):
        x = randrange(120, self.cx - 120)
        y = randrange(120, self.cy - 120)

        self.target_agent = TargetAgent(x, y, max_speed=100, world=self)
