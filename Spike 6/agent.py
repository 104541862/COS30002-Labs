...
class Agent(object):
    ...
    def __init__(self, world=None, scale=30.0, speed_scale=30.0, mass=1.0, color="LIGHT_BLUE", mode='seek', is_prey=False):
        ...
        self.force = Vector2D()
        self.accel = Vector2D()
        self.max_speed = 20.0 * speed_scale
    ...
    def calculate(self, delta):
        ...
        if mode == 'pursuit':
            ...
            force = self.flock() + self.wander(self._delta)
        else:
            force = Vector2D()
        
        force += self.avoid_rocks()
        force += self.avoid_walls()
    ...
    def update(self, delta):
        ...
        # 6. Handle collisions between rocks
        self.resolve_rock_collisions()

        # 7. Handle collisions between agents
        ...
        # 10. Clamp position to world bounds (optional if using wrap-around)
        self.pos.x = max(0, min(self.world.cx, self.pos.x))
        self.pos.y = max(0, min(self.world.cy, self.pos.y))
    ...
    def wander(self, delta):
        ...
        return self.arrive(world_target, 'normal')
    ...
    def avoid_walls(self):
        ...
        margin = 80
        turn_force = self.max_force * 0.8

        force = Vector2D()

        # LEFT WALL
        if self.pos.x < margin:
            force.x += turn_force * (1 - self.pos.x / margin)

        # RIGHT WALL
        elif self.pos.x > self.world.cx - margin:
            dist = self.world.cx - self.pos.x
            force.x -= turn_force * (1 - dist / margin)

        # BOTTOM WALL
        if self.pos.y < margin:
            force.y += turn_force * (1 - self.pos.y / margin)

        # TOP WALL
        elif self.pos.y > self.world.cy - margin:
            dist = self.world.cy - self.pos.y
            force.y -= turn_force * (1 - dist / margin)
    ...
    def resolve_agent_collisions(self):
        """Hard separation between agents, except hunter-prey interaction."""

        min_dist = self.scale.x * 1.2
        ...
        for other in self.world.agents:
            ...
            # NEVER block hunter from prey
            if (self.is_prey and other is self.world.hunter) or (self is self.world.hunter and other.is_prey):
                continue

            # normal collision resolution for all other agent pairs
            if dist < min_dist:
                overlap = min_dist - dist
                correction = to_self.normalise() * overlap

                self.pos += correction * 0.5
                other.pos -= correction * 0.5

    def get_nearby_prey(self, radius=200):
        neighbours = []
        for agent in self.world.agents:
            if agent is self:
                continue
            if agent is self.world.hunter:
                continue

            to_agent = agent.pos - self.pos
            if to_agent.length() < radius:
                neighbours.append(agent)

        return neighbours
    
    def get_nearest_prey(self):
        nearest = None
        nearest_dist = float('inf')

        for agent in self.world.agents:
            if not getattr(agent, "is_prey", False):
                continue

            d = (agent.pos - self.pos).length()
    ...
    def separation(self, neighbours):
        ...
        for n in neighbours:
            ...
            if dist > 0:
                force += diff.normalise() / (dist * dist)
                count += 1
        ...
        if count > 0:
            ...
            if force.length() > 0:
                return force.normalise() * self.max_speed - self.vel
    ...
    def cohesion(self, neighbours):
        ...
        centre /= len(neighbours)

        to_center = centre - self.pos
        return to_center * 0.01
    ...
    def alignment(self, neighbours):
        ...
        avg_vel /= len(neighbours)

        return (avg_vel - self.vel) * 0.5
    
    def hunter_threat(self):
        ...
        fear_radius = self.world.fear_radius

        if dist >= fear_radius:
            return 0.0

        return 1.0 - (dist / fear_radius)
    
    def flock(self):
        neighbours = self.get_nearby_prey()

        fear = self.hunter_threat()
        ...
        # SAFE STATE (no hunter nearby)
        if fear <= 0.01:
            return sep * self.world.sep_weight + coh * self.world.coh_weight + ali * self.world.ali_weight

        # THREAT STATE (hunter nearby)
        hunter = self.world.hunter
        away_from_hunter = Vector2D()

        if hunter:
            away_from_hunter = (self.pos - hunter.pos).normalise()

        sep *= (self.world.sep_weight + fear)        # increase separation to scatter
        coh *= (self.world.coh_weight - fear)        # collapse group structure
        ali *= (self.world.ali_weight - fear * 0.8)

        flee_force = away_from_hunter * self.max_speed

        return (sep * self.world.sep_weight + coh * self.world.coh_weight + ali * self.world.ali_weight + flee_force * fear)
